 ...
 *      Author: markin
 *
 *      HXD091D Drivers  i2c:SDA -GPIO_NUM_25
 *      				     SCL -GPIO_NUM_26
 *      				     BUSY -GPIO_NUM_27
 */
...
czx_vu32	irda_delay_1ms	=	FALSE;
//GPIO_InitTypeDef IRDA_InitStructure;

//#define portTICK_PERIOD_MS	portTICK_RATE_MS		( ( TickType_t ) 1000 / configTICK_RATE_HZ )
...
/*	io_conf.intr_type = GPIO_PIN_INTR_DISABLE;
    ...
    gpio_set_direction(IRDA_SDA_PIN,GPIO_MODE_INPUT_OUTPUT_OD);
...
/*	io_conf.intr_type = GPIO_PIN_INTR_DISABLE;
    ...
    gpio_set_direction(IRDA_SCL_PIN,GPIO_MODE_INPUT_OUTPUT_OD);
...
/*	io_conf.intr_type = GPIO_PIN_INTR_DISABLE;
    ...
    gpio_pad_select_gpio(IRDA_BUSY_PIN);
    gpio_set_direction(IRDA_BUSY_PIN,GPIO_MODE_INPUT_OUTPUT_OD);
 //   gpio_set_pull_mode(IRDA_BUSY_PIN,GPIO_PULLUP_ONLY);
}
...
/*	static const char *TAG = "hxd_ack";*/
	...
	IRDA_delay_10us(IRDA_DELAY_40US);
	for(i=0;i<len;i++){
	/*	uint8_t ack=*/IRDA_write(d[i]);
//		ESP_LOGI(TAG,"ack=%d",ack);
		IRDA_delay_10us(IRDA_DELAY_40US);
	}
	IRDA_delay_10us(IRDA_DELAY_40US);
	IRDA_stop();
...
void		IRDA_learn_start(void){//Ñ§Ï°¿ªÊŒ
	IRDA_open();
	IRDA_delay_10us(IRDA_DELAY_40US);
	IRDA_SCL_L();
	IRDA_delay_10us(IRDA_DELAY_40US);
...
czx_u8		IRDA_Modify_wave(czx_vu32	*wavedat,czx_vu32	*indatsum){
	...
	if(wavedat[0]==0){
		...
		receive_time--;
		cnt4=5;
		if(wavedat[5]>=0x80){				//wjs;Ïàµ±ÓÚ9msµÄÊýŸÝ
			cnt4=7;
			if(wavedat[6]<253){
				wavedat[6]+=3;
			}else{
	...
	if(wavedat[0]==0){
		...
		for(cnt2=5;cnt2<*indatsum+1;cnt2++){
			...
			if((cnt3&0x01)==0){
				...
				if(waveld1>128)
					return	1;	//ÔØ²š²šÐÍŽóÓÚ2048us--error
			}else{
				waveld1-=waveld2;
				if(waveld1<=2){
					waveld1=1;
...
czx_u32	IRDA_acmpbandc(czx_vu32	cmpdat1,czx_vu32	cmpdat2,czx_vu32	cmpdat3)
{
	//A==0:AÔÚB,CÖ®Œä;==0F0H:no,>œÏŽóµÄÊý;==0Fh:no,<œÏÐ¡µÄÊý;
...
czx_u8 IRDA_send_remote_study_data(czx_vu8 *cmd_data, czx_vu16 cmd_lnth)		//wjs;cmd_lnth=109
{
	...
	for(temp=0; temp<cmd_lnth; temp++)			///wjs;cmd_lnth £œ109
		sendlrndat[temp+1] = cmd_data[temp];		//wjs;cmd_data[]£º110£ÂŽŠÀíºó£¬ÕâÊýŸÝÖ»ÓÐÕâÀïÓÐÓÃ£»£»sendlrndat[]£º110£Â
	...
	toggle=IRDA_keytogglebit(sendlrndat,&sendlrndatsum);	//wjs;°üÀšÁËž÷ÖÖÊýŸÝŽŠÀí
	...
	return cmd_lnth;
//	templnth = 112;				;;wjs;2012-7-10 21:36;Ô­ÀŽÊÇÍš¹ý£É£ÏÍšÑ¶·¢³öµÄ²¿·Ö
//
...
czx_u8		IRDA_get_remote_study_data(czx_vu8 *cmd_data)
{
	...
	temp = IRDA_rx_data(readtempbuf);		//·¢Ñ§Ï°ÃüÁîÇ°£¬ÒªÇåreadtempbuf
	...
	else
	{
		...
		{
			irprocessbuf[temp] = (czx_vu32)readtempbuf[temp];		///wjs;readtempbuf,irprocessbufÔ­ÊŒ110£Â:ÎŽŽŠÀí
		}
		datalength = 109+1;
		if(0!=(IRDA_compdata(irprocessbuf, &datalength)))
		{
			...
			{
				cmd_data[temp] = (czx_vu8 )irprocessbuf[temp+1];		//wjs;cmd_data[]:110BŽŠÀí¹ý³ÌÖÐµÄÊýŸÝ:109B
			}
		}
...
{
	 czx_vu8 	tempw;
	 czx_vu8         tempct;			//ÔöŒÓÕâžö±äÁ¿
	czx_vu8	data_check=0;			//ÔöŒÓÕâžö±äÁ¿
	czx_vu32 i = 0;
	//unsigned char	 g_remote_study_data_buf[112];

	///////=================================================================//wjs;ÎªœâŸöµÃµœÑ§Ï°ŽíÎóÊýŸÝÎÊÌâ
...
//	learn_data_out[i]=0;
		...
		{
			learn_data_out[tempw] = g_i2c_cmd_buffer[tempw];		//wjs;µÃµœµÄÑ§Ï°ÊýŸÝ
		}
					//*********************************************ÔöŒÓ£ºŽíÎóÊýŸÝ£¬ÊýŸÝÇå0
		for(tempw=10; tempw<15; tempw++)
		{
			data_check  += (learn_data_out[tempw]);
		}
		if(learn_data_out[3]<5||learn_data_out[5]<0x10||learn_data_out[5]>0x80)	//Ô­ÀŽ£ºif(!data_check)
		{
			for(int i = 0; i< 112;i++){
           	//	learn_data_out[i]=0;
			}
...
UINT16 I2CReadData(UINT8* pbData)
{
	...
	while (i--)
	{
		...
		readdata |= IRDA_GET_DATA();  //读不到时可以这样试：readdata |= GetDINStatus()?0x01:0x00;
//		readdata |= IRDA_GET_DATA()?0x01:0x00;
		IRDA_SCL_L();
		IRDA_delay_10us(IRDA_DELAY_40US);
		IRDA_delay_10us(IRDA_DELAY_40US);		//wjs;加delay
	}
	IRDA_SCL_L();
	IRDA_delay_10us(IRDA_DELAY_40US);
	...
	*pbData = readdata;
	...
	IRDA_delay_10us(IRDA_DELAY_40US);
	IRDA_delay_10us(IRDA_DELAY_40US);		//wjs;1G以上的系统要加,test
...
//=============================
//===================
uint8_t readI2C(uint8_t* readtempbuf)     //UINT8
{
	UINT8 bValue;
	UINT8 i=0;
	UINT8 checksum;
